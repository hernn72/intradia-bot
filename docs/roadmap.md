...
| C-09 | **Caché local de barras de sesión cerrada ya validadas (D-40, D-41)**: el proveedor **retira** por la noche una barra que ya sirvió, y el bot la vio la tarde anterior, así que la caché va **primero** y la segunda fuente queda **condicionada** a una cifra que la propia ficha produce: sesión exigible + nunca observada + no entregada. No pasa por B-00 (no es fuente externa); la segunda fuente sí, si llega | PENDIENTE, **ficha escrita** (2026-09-20) | — | **T-018** |
...
| A-02 | Rehacer P2.3, P2.4 y P2.5 una sola vez sobre `071ddb2b…`, con RS alineada y línea 0; decidir el RR en el score → **GATE P2** | **EN_REVISION** — implementada el 2026-09-21 (D-42), evidencia en `evidence/2026-09-21-T-013-laboratorio-rehecho/`. Las tres poblaciones reproducen sus hashes; `pre-d31` devuelve las 121.786 señales de agosto y solo 194 (0,16 %) cambian de banda, así que toda la diferencia con lo vigente es de población. Con el primario de INV-14 el veredicto global pasa de `LIMITADA`/`MEDIUM` a **`INSUFICIENTE`/`LOW`** y ninguna banda es concluyente. **GATE P2 sigue sin cruzar**: falta el requisito 6, la decisión del RR, abierta como **OD-11** | GATE L0 | **T-013** |
